...
const RepositoryTable: React.FC<RepositoryTableProps> = ({ repositories, onRowClick, loading }) => {
	...
	const [sortDirection, setSortDirection] = React.useState<'asc' | 'desc'>('asc'); // Направление сортировки
						...
						<Table className={styles.table}>
							<TableHead>
								<TableRow>
									...
									<TableCell onClick={() => handleSort('name')}>Название</TableCell>
									<TableCell onClick={() => handleSort('language')}>Язык</TableCell>
									<TableCell onClick={() => handleSort('forksCount')}>Число форков</TableCell>
									<TableCell onClick={() => handleSort('starsCount')}>Число звёзд</TableCell>
									<TableCell onClick={() => handleSort('updatedAt')}>Дата обновления</TableCell>
								</TableRow>
							</TableHead>
							<TableBody>
								{/* Отображение строк таблицы с репозиториями */}
								{sortedRepositories.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((repo) => (
									<TableRow key={repo.id} hover onClick={() => onRowClick(repo.id)} className={styles.tableRow}>
